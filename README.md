...
It also accepts a percent-encoded HTTP/HTTPS URL through `mpv://`, for example:
...
`--force` replaces the currently registered `ush` and `mpv` handlers. Their
previous commands are saved in the registry and restored by:
...
Only `ush://MPV` and `mpv://<percent-encoded HTTP/HTTPS URL>` are accepted;
other `ush` targets and local file paths arriving from a web protocol request
are rejected.
